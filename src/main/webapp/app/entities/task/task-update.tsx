...
export class TaskUpdate extends React.Component<ITaskUpdateProps, ITaskUpdateState> {
  ...
  render() {
    const { taskEntity, tODOLists, loading, updating } = this.props;
    ...
    return (
      <div>
        <Row className="justify-content-center">
          <Col md="8">
            <h2 id="betterApp.task.home.createOrEditLabel">Create or edit a Task</h2>
          </Col>
        </Row>
        <Row className="justify-content-center">
          <Col md="8">
            {loading ? (
              ...
              <AvForm model={isNew ? {} : taskEntity} onSubmit={this.saveEntity}>
                ...
                <AvGroup>
                  <Label for="task-tODOList">T ODO List</Label>
                  <AvInput id="task-tODOList" type="select" className="form-control" name="tODOList.id">
                    <option value="" key="0" />
                    {tODOLists
